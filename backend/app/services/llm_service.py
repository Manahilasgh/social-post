...
from app.services.platform_config import get_platform_config, DEFAULT_PLATFORM
...
def generate_post_copy(query: str, news_results: list[dict], platform: str = DEFAULT_PLATFORM) -> dict:
    """
    Given a topic and fetched news results, ask the LLM to produce:
    headline, description, story_description, hashtags (list[str])
    `description` is constrained to fit the target platform's character limit.
    ...
    system_prompt = (
        ...
        f"description: a caption for {platform}, MUST be under {max_chars} characters total "
        "(including spaces and punctuation) — this is a hard limit, count carefully.\n"
        "story_description: slightly longer, 2-3 sentences, for story/card format (not platform-limited).\n"
    ...
    # Safety net: LLMs don't always respect character limits precisely,
    # so hard-truncate as a backstop rather than trusting the prompt alone.
